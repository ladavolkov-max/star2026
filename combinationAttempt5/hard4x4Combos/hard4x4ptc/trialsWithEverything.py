...
class TrialBatch:
    ...
    def runBatch(self):
        ...
        visionPreTrainingResults = self.runPretrainingVision()
        spatialPreTrainingResults = self.runPretrainingSpatial()
        #running the overall training of the combined CA
        results = self.runOverallTraining()
        #returning all the data
        return visionPreTrainingResults, spatialPreTrainingResults, results

    #-----------------------------------------------------------------------------------------------------------------
    #pretraining for vision
    def runPretrainingVision(self):
        print("Running pretraining for vision CA")
        #run numTrials trials for just the vision CA
        results = []
        #---------------------------------------outer loop
        for trial in range(self.__numTrials): 
            print(f"VISION trial {trial}")
            ...
            for step in range(self.__numSteps): 
                ...
                if response == b'0': #do nothing
                    ...
                    if (self.__locX == self.__locBarX and
                        ...
                        successfulPress = True
                        # lever pressed, send reward signal
                        self.__pidSpatial.stdin.write(b"21/1\n")
        ...
        if((1 <= self.__locX <= 2 and self.__locY == 4) or
           ...
           (self.__locX == 3 and self.__locY == 1)):
            regionNum = 1
             ...
             (self.__locX == 3 and 2 <= self.__locY <= 4) or
             (self.__locX == 4 and self.__locY == 2)):
            regionNum = 2
        elif(self.__locX == 4 and 3 <= self.__locY <= 4):
            regionNum = 3
