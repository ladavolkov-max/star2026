        ...
        for trial in range(self.__numTrials): 
            ...
            for step in range(self.__numSteps): 
                ...
                if response == b'0': #do nothing
                    #send small reward signal (not running into wall)
                    ...
                    #send small reward signal (not running into wall)
                    self.__pidSpatial.stdin.write(b"17/05\n")
                    self.__pidSpatial.stdout.readline()
                    self.__dir += 1
                    #wrap around if necessary
                    if self.__dir == 5:
            ...
            nonPunishedActions.append(nonPunishedCount)
            zeroActions.append(zeroCount)
            #reset ca 
            self.__pidSpatial.stdin.write(b"0/1\n")
            self.__pidSpatial.stdout.readline()
        #---------------------------------------outer loop
        #stop the ca and give the results
        return results, nonPunishedActions, zeroActions
