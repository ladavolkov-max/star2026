...
class TrialBatch:
    ...
    def runBatch(self):
        ...
        for trial in range(self.__numTrials): 
            ...
            for step in range(self.__numSteps): 
                ...
                #sending input to decision CA
                inputSymbol, inputConfidence = self.getDecisionScenario(responseVision, responseSpatial, confidenceVision, confidenceSpatial)
                inputString = f"{inputSymbol}/{inputConfidence}\n"
                try:
                ...
                if response == b'0': #do nothing
                    ...
                    if (self.__locX == self.__locBarX and
                        ...
                        self.__pidDecision.stdin.write(b"26/1\n")
    ...
    #-----------------------------------------------------------------------------------------------------------------
    #method to get the decision scenario based on the sensor outputs
    def getDecisionScenario(self, visionResponse, spatialResponse, visionConf, spatialConf):
        wordsToNum = {
            "vision0spatial0" : 1,
            "vision0spatial1" : 2,
            "vision0spatial2" : 3,
        ...
        decisionNum = 0
        visionNum = visionResponse.decode('utf-8')
        spatialNum = spatialResponse.decode('utf-8')
        string = f"vision{visionNum}spatial{spatialNum}"
        decisionNum = wordsToNum[string]
        if decisionNum == 0:
            print(f"ERROR! unmapped combination: vision - {visionNum.decode('utf-8')}, spatial - {spatialNum.decode('utf-8')}")
        averageConfidence = (visionConf + spatialConf) / 2
        return decisionNum, averageConfidence
