...
class TrialBatch:
    ...
    def __init__(self,
            ...
            #establishing starting location
            ...
            self.__caPath = os.path.join(self.__scriptDir, "ca.macos") #path of the CA executable
        
            #starting up the different CAs
            self.__cfgPathImmediateVision = os.path.join((os.path.join(self.__scriptDir, "configs")), "immediateVision.cfg") #path of the configuration file
            self.__pidImmediateVision = Popen([self.__caPath, self.__cfgPathImmediateVision], stdin=PIPE, stdout=PIPE, bufsize=0) #starting up the ca process
            self.__cfgPathDistanceVision = os.path.join((os.path.join(self.__scriptDir, "configs")), "distanceVision.cfg") #path of the configuration file
            self.__pidDistanceVision = Popen([self.__caPath, self.__cfgPathDistanceVision], stdin=PIPE, stdout=PIPE, bufsize=0) #starting up the ca process
            self.__cfgPathStrategicTopological = os.path.join((os.path.join(self.__scriptDir, "configs")), "strategicTopological.cfg") #path of the configuration file
            self.__pidStrategicTopological = Popen([self.__caPath, self.__cfgPathStrategicTopological], stdin=PIPE, stdout=PIPE, bufsize=0) #starting up the ca process
            self.__cfgPathImmediateSpatial = os.path.join((os.path.join(self.__scriptDir, "configs")), "immediateSpatial.cfg") #path of the configuration file
            self.__pidImmediateSpatial = Popen([self.__caPath, self.__cfgPathImmediateSpatial], stdin=PIPE, stdout=PIPE, bufsize=0) #starting up the ca process
            self.__cfgPathDecision = os.path.join((os.path.join(self.__scriptDir, "configs")), "decision") #path of the configuration file
    ...
    #-----------------------------------------------------------------------------------------------------------------
    #running the batch with the pretraining and then the overall training of the combined
    def runBatch(self):
        #running the pretraining for the vision and spatial CAs
        immediateVisionPreTrainingResults = self.immediateVisionPreTraining()
        distanceVisionPreTrainingResults = self.distanceVisionPreTraining()
        strategicTopologicalPreTrainingResults = self.strategicTopologicalPreTraining()
        immediateSpatialPreTrainingResults = self.immediateSpatialPreTraining()
        decisionPreResults = self.decisionPreTraining()
        #running the overall training of the combined CA
        results = self.overallTraining()
        #returning all the data
        return (
            immediateVisionPreTrainingResults,
            distanceVisionPreTrainingResults,
            strategicTopologicalPreTrainingResults,
            immediateSpatialPreTrainingResults,
            decisionPreResults,
            results
        )

    #-----------------------------------------------------------------------------------------------------------------
    #pretraining for vision
    def immediateVisionPreTraining(self):
        print("PreTraining Immediate Vision")
        #key is the visionScenario number, value is the correct response number in that situation
        #pretraining only based off wall scenarios bc hasnt encountered bar before
        correctResponses = {
            1: 1, #---4
            2: 2, #--34
            3: 3, #1--4
        ...
        results = []
        #---------------------------------------outer loop
        for trial in range(1): #no large reward at the end so only need 1 long trial to learn
            #tracking the number of steps taken in the trial
            stepsTaken = 0
            #---------------------------------------inner loop
            for step in range(100): 
                visionScenario = random.randint(1, 20)
                try:
                    self.__pidVision.stdin.write((str(visionScenario) + '/1\n').encode('utf-8'))
                except BrokenPipeError:
                    print("broken pipe error, ca exited early (immediate vision)")
                    raise
                #gettting the CA's response
                responseFullLine = self.__pidVision.stdout.readline()
                response = responseFullLine[0:1]
                #next step, keeping track for trial results
                stepsTaken += 1
                if response == b'0': #----
                    #resp
                    if visionScenario == 1 or visionScenario == 7:
                        self.__pidVision.stdin.write(b"0/1\n")
                        self.__pidVision.stdout.readline()
                elif response == b'1': #---4
                elif response == b'2': #--34
                elif response == b'3': #1--4
                elif response == b'4': #-2-4
                else:
    ...
    #-----------------------------------------------------------------------------------------------------------------
    #pretraining for spatial
    def runPretrainingSpatial(self):
        print("Running pretraining for spatial CA")
        #run numTrials trials for just the spatial CA
        results = []
        #---------------------------------------outer loop
        for trial in range(self.__numTrials): 
            print(f"SPATIAL trial {trial}")
            ...
            for step in range(self.__numSteps): 
                ...
                response = responseSpatialFullLine[0:1]
                #next step, keeping track for trial results
                stepsTaken += 1
    ...
    #-----------------------------------------------------------------------------------------------------------------
    #pretraining for spatial
    def runOverallTraining(self):
        print("Running training with all 3 CAs")
        #run numTrials trials for the combined CAs
        results = []
        #---------------------------------------outer loop
        for trial in range(self.__numTrials): 
            print(f"COMBINED trial {trial}")
            #random location and dir setup
            #resetting ca to start the new trial
            if self.__randomizeLoc:
                self.generateRandLoc()
    ...
    def getSpatialScenario(self):
        ...
        regionNum = 0
        if((1 <= self.__locX <= 2 and self.__locY == 4) or
           (1 <= self.__locX <= 2 and self.__locY == 2) or
           (self.__locX == 3 and self.__locY == 1)):
            regionNum = 1
        elif((1 <= self.__locX <= 2 and self.__locY == 3) or
             (3 <= self.__locX <= 4 and 2 <= self.__locY <= 4)):
            regionNum = 2
        elif(1 <= self.__locX <= 2 and self.__locY == 1):
            regionNum = 3
        elif(self.__locX == 4 and self.__locY == 1):
            regionNum = 4
        else:
