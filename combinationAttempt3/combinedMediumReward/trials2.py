...
class TrialBatch:
    ...
    def __init__(self,
        ...
        self.__previousRegion = -1 #track region change for medium reward
    ...
    def runBatch(self):
        ...
        for trial in range(self.__numTrials): 
            ...
            for step in range(self.__numSteps): 
                ...
                if self.__previousRegion != -1:
                    #check if region has changed, if so send medium reward
                    spatialScenario = self.getSpatialScenario()
                    if 1 <= spatialScenario <= 4:
                        currentRegion = 1
                    elif 5 <= spatialScenario <= 8:
                        currentRegion = 2
                    elif 9 <= spatialScenario <= 12:
                        currentRegion = 3
                    else:
                        currentRegion = 4
                    if currentRegion != self.__previousRegion:
                        self.sendMediumReward(0.25, 0.25, 0.25)
                    self.__previousRegion = currentRegion
                ...
                if response == b'0': #do nothing
                    self.sendSmallReward() #send small reward to sensors for not running into the wall
    ...
    #-----------------------------------------------------------------------------------------------------------------
    #method for sending small reward to sensors (for not running into the wall)
    def sendSmallReward(self):
        #send small rewards to sensors
        self.__pidVision.stdin.write(b"21/0.1\n")
        self.__pidVision.stdout.readline()
        self.__pidSpatial.stdin.write(b"17/0.05\n")
        self.__pidSpatial.stdout.readline()

    #-----------------------------------------------------------------------------------------------------------------
    #method for sending medium reward
    def sendMediumReward(self, strengthVision, strengthSpatial, strengthDecision):
        #send medium reward to all CAs
        self.__pidVision.stdin.write(b"21/{strengthVision}\n")
        self.__pidVision.stdout.readline()
        self.__pidSpatial.stdin.write(b"17/{strengthSpatial}\n")
        self.__pidSpatial.stdout.readline() 
        self.__pidDecision.stdin.write(b"11/{strengthDecision}\n")
        self.__pidDecision.stdout.readline()