...
class TrialBatch:
    ...
    def getSpatialScenario(self):
        ...
        regionNum = 0
        if(
                1 <= self.__locX <= 4 and 2 <= self.__locY <= 3 or
                self.__locX == 4 and self.__locY == 1
        ):
            regionNum = 1
        elif(
                1 <= self.__locX <= 2 and 4 <= self.__locY <= 5 or
                3 <= self.__locX <= 5 and self.__locY == 4 or
                self.__locX == 5 and 2 <= self.__locY <= 4
        ):
            regionNum = 2
        elif(
            1 <= self.__locX <= 2 and self.__locY == 1
        ):
            regionNum = 3
        elif(
            3 <= self.__locX <= 5 and self.__locY == 5
        ):
            regionNum = 4
        elif(
            self.__locX == 5 and self.__locY == 1
        ):
            regionNum = 5
        else:
